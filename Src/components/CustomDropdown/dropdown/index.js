...
export default class Dropdown extends PureComponent {
  ...
  renderBase(props) {
    ...
    title = null == title || 'string' === typeof title?
      title:
      String(title);
    // alert(JSON.stringify(props, undefined, 2));
    return (
      // <TextField
      //   label=''
      ...
      // />
      <View style={{ width:'100%', borderBottomWidth:1, borderBottomColor:'gray', alignItems:'center', justifyContent:'space-between', flexDirection:'row'}}>
        <View style={{flex:1}}>
        <Text style={{fontFamily: 'Avenir-Black',color:'#00b7ce'}} numberOfLines={1}>{title}</Text>
        </View>
        {/* <Icons name="arrow-back" size={30} color="black" style={{marginLeft:2}} /> */}
        <Icons name='arrow-drop-down' color='gray' size={24} />
      </View>
    );
  }
